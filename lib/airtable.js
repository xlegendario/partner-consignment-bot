...
export async function createSaleAndDecrement({ inventoryId, orderRecId, finalPrice }) {
  ...
  // Sales VAT/price:
  // If seller is NL AND inventory VAT is VAT0 → VAT21 and uplift by 21%; else keep as-is.
  let vatTypeOut = vatTypeName || "MARGIN";
  let finalOut   = round2(finalPrice);
  if (vatTypeName === "VAT0" && isNL(sellerCountry)) {
    vatTypeOut = "VAT21";
