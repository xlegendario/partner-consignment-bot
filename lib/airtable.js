...
const {
  ...
  FIELD_INV_LINKED_ORDER = "Linked Order",            // linked to Orders table (or text in your base)
...
export async function linkInventoryToOrder(inventoryRecordId, orderRecId) {
  ...
  try {
    await airtableRequest("PATCH", tablePath, {
      fields: { [FIELD_INV_LINKED_ORDER]: [{ id: orderRecId }] }
    });
    return;
  } catch (e) {
  ...
  await airtableRequest("PATCH", tablePath, {
    fields: { [FIELD_INV_LINKED_ORDER]: orderRecId }
  });
}
