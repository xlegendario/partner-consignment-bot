...
export async function setOrderMatchedStatus(orderRecId, matchedLabel = "Matched") {
  // If your status field/name uses envs, keep using them. Otherwise:
  const FIELD_ORDER_STATUS = process.env.FIELD_ORDER_STATUS || "Fulfillment Status";
  const body = { fields: { [FIELD_ORDER_STATUS]: { name: matchedLabel } } };
  await airtableRequest(
    "PATCH",
    `${encodeURIComponent(AIRTABLE_TABLE_ORDERS)}/${orderRecId}`,
    body
  );
}
