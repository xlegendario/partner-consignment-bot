...
export async function createSaleAndDecrement({ inventoryId, orderRecId, finalPrice }) {
  ...
  // Linked record IDs (as arrays of rec ids — same as your earlier working version)
  ...
  // Convert VAT0 -> VAT21 for Sales row (seller sells to your NL company)
  const vatTypeOut = (String(vatTypeName).toUpperCase() === "VAT0") ? "VAT21" : (vatTypeName || "Margin");

  // 2) Create Sales row (SHAPES EXACTLY LIKE YOUR PREVIOUS WORKING VERSION)
  const saleFields = {
    ...
    [FIELD_SALE_SKU_LINK]:     skuLinkId ? [skuLinkId] : undefined,      // <-- ["rec..."]
    [FIELD_SALE_SELLER_LINK]:  sellerLinkId ? [sellerLinkId] : undefined,// <-- ["rec..."]
    [FIELD_SALE_ORDER_LINK]:   orderRecId ? [orderRecId] : undefined,    // <-- ["rec..."]
  };
