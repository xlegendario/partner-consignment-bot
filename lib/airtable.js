...
export async function linkInventoryToOrder(inventoryRecordId, orderRecordId) {
  if (!AIRTABLE_INV_LINKED_ORDER) {
    throw new Error("Set AIRTABLE_INV_LINKED_ORDER to the field name (e.g., 'Linked Order').");
  }
  // If your field allows only a single link, this will overwrite with the current order.
  // If it allows multiple, this will set a single-element array.
  return base(AIRTABLE_TABLE_INVENTORY).update([
    {
