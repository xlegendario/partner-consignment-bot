...
export async function initDiscord() {
  ...
  client = new Client({
    intents: [GatewayIntentBits.Guilds], // buttons don't need Message Content intent
  });
...
----------------------------------------------------------- */
// Confirm whenever seller ask is <= our adjusted max (target is informational now)
...
export async function sendOfferMessageGateway({
  ...
  if (!confirmCase) {
    lines.push(
      "",
      "**Our Offer**",
      offerPrice != null ? `€${Number(offerPrice).toFixed(2)} ${ourPriceLabel}` : "—"
    );
  }
  ...
  const embed = {
    title,
    description,
    color: confirmCase ? 0x2ecc71 : 0xf1c40f, // green for confirm, yellow for offer
