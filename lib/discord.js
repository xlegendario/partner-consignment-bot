...
export async function sendOfferMessageGateway({
  ...
  productName, sku, size, suggested, adjustedMax,
  vatType, sellerCountry, clientCountry, sellerVatRatePct, quantity
}) {
  ...
  const lines = [
    ...
    "**Size**\n" + (size ?? "—"),
    "**Quantity**\n" + (quantity ?? 0),
    "**Your Price**\n" + (suggested != null ? `${euro(suggested)} ${yourTag}` : "—"),
  ];
  if (!confirmCase) {
    lines.push("", "**Our Offer**", offerPrice != null ? `${euro(offerPrice)} ${ourTag}` : "—");
  }
  ...
  const components = [{
    ...
    components: [
      {
        type: 2, style: 3,
        label: confirmCase ? "Confirm (I have this pair)" : `Accept Offer ${euro(offerPrice)}`,
        custom_id: `confirm|${orderRecId}|${sellerId}|${inventoryRecordId}|${typeof offerPrice === "number" ? offerPrice : 0}`
      },
      {
        type: 2, style: 4,
        label: "Deny (Not available)",
        custom_id: `deny|${orderRecId}|${sellerId}|${inventoryRecordId}|${typeof offerPrice === "number" ? offerPrice : 0}`
      }
    ]
  }];
