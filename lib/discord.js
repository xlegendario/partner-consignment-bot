...
export async function onButtonInteraction(handler) {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    await interaction.deferUpdate().catch(() => {}); // ack once
    try {
...
/* ---------- Channel helpers (category per seller) ---------- */
...
async function getChannelIdForSeller(sellerNameOrId, kind) {
  ...
  if (!categoryId) {
    if (String(ALLOW_CHANNEL_CREATE).toLowerCase() === "true") {
      ...
      throw new Error(`Missing category "${sellerNameOrId}"`);
    }
  }
...
export async function sendOfferMessageGateway({
  ...
  productName, sku, size, quantity = 1,
  normalizedSuggested, normalizedTarget, normalizedMax,
  vatType, sellerCountry, clientCountry
}) {
  // Decide using normalized values only
  const confirmCase = isNumber(normalizedSuggested) && isNumber(normalizedMax) && normalizedSuggested <= normalizedMax;
  const offerPrice  = confirmCase
    ? normalizedSuggested
    : (isNumber(normalizedMax) ? normalizedMax : null);

  const labelTag = vatSuffix({ vatType, sellerCountry, clientCountry });
  ...
  const lines = [
    ...
    "**Your Price**", isNumber(normalizedSuggested) ? `€${normalizedSuggested.toFixed(2)} ${labelTag}` : "—",
  ];

  if (!confirmCase) {
    lines.push("",
      "**Our Offer**",
      isNumber(offerPrice) ? `€${offerPrice.toFixed(2)} ${labelTag}` : "—"
    );
  }
  ...
  if (!channelId) throw new Error(`[Discord] No channelId resolved for seller="${sellerName || sellerId}" kind=${kind}`);
  ...
  const components = [{
    ...
    components: [
      {
        type: 2, style: 3,
        label: confirmCase ? "Confirm (I have this pair)" : `Accept Offer €${isNumber(offerPrice) ? offerPrice.toFixed(2) : "—"}`,
        custom_id: `confirm|${orderRecId}|${sellerId}|${inventoryRecordId}|${isNumber(offerPrice) ? offerPrice : 0}`
      },
      {
        type: 2, style: 4,
        label: "Deny (Not available)",
        custom_id: `deny|${orderRecId}|${sellerId}|${inventoryRecordId}|${isNumber(offerPrice) ? offerPrice : 0}`
      }
    ]
  }];
