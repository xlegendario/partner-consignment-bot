...
export async function initDiscord() {
  ...
  client = new Client({
    intents: [
      // Keep intents minimal to avoid “Used disallowed intents”
...
export async function onButtonInteraction(handler) {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    try {
      ...
      // Ack silently so users don’t see “This interaction failed”
...
/* -----------------------------------------------------------
   Pricing helpers + labels
----------------------------------------------------------- */
// Confirm whenever seller ask ≤ adjustedMax
