...
export async function sendOfferMessageGateway({
  ...
  size,
  suggested,
  adjustedMax,           // already normalized value you compare against
  vatType,
  sellerCountry,
  clientCountry,
  quantity
}) {
  const confirmCase = (typeof suggested === "number") &&
  ...
  const label = (vt, sc, cc) => {
    const v = String(vt || "").toUpperCase();
    const isNL = s => String(s || "").toLowerCase().includes("nether");
    const bothNL = isNL(sc) && isNL(cc);
    if (v.includes("MARGIN")) return "(Margin)";
    if (v.includes("VAT21"))  return "(VAT 21%)";
    if (v.includes("VAT0"))   return bothNL ? "(VAT 21%)" : "(VAT 0%)";
    return "";
  };

  const yourTag = label(vatType, sellerCountry, clientCountry);
  const ourTag  = label(vatType, sellerCountry, clientCountry);

  // ✅ Top line OUTSIDE the embed
  const topLine = confirmCase
  ...
  if (!channelId) throw new Error(`[Discord] No channelId for seller="${sellerName || sellerId}" kind=${kind}`);
  ...
  const embed = {
    title: embedTitle,                 // 👈 now the title
    description,
  ...
  const res = await fetch(`${API}/channels/${channelId}/messages`, {
    ...
    body: JSON.stringify({
      content: topLine,                // 👈 outside-embed line
      embeds: [embed],
      components
    })
  });
