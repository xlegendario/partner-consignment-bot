...
export async function onButtonInteraction(handler) {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    try {
      ...
      // Acknowledge silently to avoid “This interaction failed”
...
/**
 * Decide if suggested fits between adjusted [target, max].
 * `adjustedTarget` / `adjustedMax` already account for VAT logic from Airtable.
 */
function isInBetween(suggested, adjustedTarget, adjustedMax) {
  return [suggested, adjustedTarget, adjustedMax].every(n => typeof n === "number")
    && suggested >= adjustedTarget
    && suggested <= adjustedMax;
}

/**
 * Create a suffix label for a price line based on VAT regime.
 * For VAT0 + both NL we treat as VAT 21% (domestic VAT due).
 */
 ...
 * Expects pre-normalized fields from Airtable:
 *  - adjustedTarget / adjustedMax (already divided for VAT0 cross-border)
...
export async function sendOfferMessageGateway({
  ...
  // which kind of message (confirm vs offer) + offer price calculation
  const inBetween = isInBetween(suggested, adjustedTarget, adjustedMax);
  const offerPrice = inBetween
    ? suggested
  ...
  // Resolve channel (confirm vs offer)
